//! # Folio — A comprehensive PDF library for Rust
//!
...
pub mod filters {
    pub use folio_filters::*;
}

/// Convenience type alias for the library's error type.
pub type Error = folio_core::FolioError;

/// Convenience type alias for Results.
pub type Result<T> = folio_core::Result<T>;
...
pub mod prelude {
    // Document types
    pub use folio_doc::{DocInfo, Page, PdfDoc};

    // Low-level access
    pub use folio_cos::CosDoc;

    // Text extraction and search
    pub use folio_text::{SearchOptions, SearchResult, TextExtractor, TextSearch};

    // Images
    pub use folio_image::{PdfImage, add_image_to_page};

    // Annotations
    pub use folio_annot::{Annot, AnnotFlags, AnnotType};

    // Forms
    pub use folio_forms::{AcroForm, Field, FieldFlags, FieldType};

    // Navigation
    pub use folio_nav::{Action, ActionType, Bookmark, Destination, FitType};

    // Core types
    pub use folio_core::{ColorPt, FolioError, Matrix2D, PdfDate, Point, QuadPoint, Rect, Result};
}
